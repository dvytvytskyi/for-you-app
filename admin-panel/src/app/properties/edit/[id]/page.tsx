...
const usdToAed = (usd: number) => Math.round((usd * 3.67) * 100) / 100
const usdToEur = (usd: number) => Math.round((usd * 0.92) * 100) / 100
...
export default function EditPropertyPage() {
  ...
  const loadProperty = async () => {
    ...
    try {
      ...
      if (property.price) {
        setPriceValue(String(property.price))
      }
      if (property.sizeFrom !== null && property.sizeFrom !== undefined) {
        setSizeFromValue(String(property.sizeFrom))
      }
      if (property.sizeTo !== null && property.sizeTo !== undefined) {
        setSizeToValue(String(property.sizeTo))
      }
      if (property.size) {
        setSizeValue(String(property.size))
      }
      
      // Ensure facilityIds are set properly
